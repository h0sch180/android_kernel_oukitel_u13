...
static struct pinctrl_state *led0_red_gpio_en0, *led0_red_gpio_en1, *led1_green_gpio_en0, *led1_green_gpio_en1, *led2_blue_gpio_en0, *led2_blue_gpio_en1,
                             *leds_kpd_gpio_en0,*leds_kpd_gpio_en1;


int mt_leds_get_gpio_info(struct platform_device *pdev)
{
	int ret;

	printk("[led] mt_leds_get_gpio_info+++++++++++++++++\n");
	...
	if (IS_ERR(pins_default)) {
		...
		dev_err(&pdev->dev, "fwq Cannot find leds pinctrl default %d!\n", ret);
	}
	led0_red_gpio_en0 = pinctrl_lookup_state(pinctrl1, "led0_red_gpio_en0");
	if (IS_ERR(led0_red_gpio_en0)) {
		ret = PTR_ERR(led0_red_gpio_en0);
		dev_err(&pdev->dev, "fwq Cannot find leds pinctrl led0_red_gpio_en0!\n");
	}
	led0_red_gpio_en1 = pinctrl_lookup_state(pinctrl1, "led0_red_gpio_en1");
	if (IS_ERR(led0_red_gpio_en1)) {
		ret = PTR_ERR(led0_red_gpio_en1);
		dev_err(&pdev->dev, "fwq Cannot find leds pinctrl led0_red_gpio_en1!\n");
	}
	led1_green_gpio_en0 = pinctrl_lookup_state(pinctrl1, "led1_green_gpio_en0");
	if (IS_ERR(led1_green_gpio_en0)) {
		ret = PTR_ERR(led1_green_gpio_en0);
		dev_err(&pdev->dev, "fwq Cannot find leds pinctrl led1_green_gpio_en0!\n");
	}
	led1_green_gpio_en1 = pinctrl_lookup_state(pinctrl1, "led1_green_gpio_en1");
	if (IS_ERR(led1_green_gpio_en1)) {
		ret = PTR_ERR(led1_green_gpio_en1);
		dev_err(&pdev->dev, "fwq Cannot find leds pinctrl led1_green_gpio_en1!\n");
	}
	led2_blue_gpio_en0 = pinctrl_lookup_state(pinctrl1, "led2_blue_gpio_en0");
	if (IS_ERR(led2_blue_gpio_en0)) {
		ret = PTR_ERR(led2_blue_gpio_en0);
		dev_err(&pdev->dev, "fwq Cannot find leds pinctrl led2_blue_gpio_en0!\n");
	}
	led2_blue_gpio_en1 = pinctrl_lookup_state(pinctrl1, "led2_blue_gpio_en1");
	if (IS_ERR(led2_blue_gpio_en1)) {
		ret = PTR_ERR(led2_blue_gpio_en1);
		dev_err(&pdev->dev, "fwq Cannot find leds pinctrl led2_blue_gpio_en1!\n");
	}
	leds_kpd_gpio_en0 = pinctrl_lookup_state(pinctrl1, "leds_kpd_gpio_en0");
	if (IS_ERR(leds_kpd_gpio_en0)) {
		ret = PTR_ERR(leds_kpd_gpio_en0);
		dev_err(&pdev->dev, "fwq Cannot find leds pinctrl leds_kpd_gpio_en0!\n");
	}
	leds_kpd_gpio_en1 = pinctrl_lookup_state(pinctrl1, "leds_kpd_gpio_en1");
	if (IS_ERR(leds_kpd_gpio_en1)) {
		ret = PTR_ERR(leds_kpd_gpio_en1);
		dev_err(&pdev->dev, "fwq Cannot find leds pinctrl leds_kpd_gpio_en1!\n");
	}        
	return 0;
}
...
int leds_gpio_output(int pin, int level)
{
	int ret =-1;

	printk("[leds]leds_gpio_output pin = %d, level = %d\n", pin, level);

	switch(pin)
	{
	case MT65XX_LED_MODE_GPIO_R: 	
           if(IS_ERR(led0_red_gpio_en0)||IS_ERR(led0_red_gpio_en1))
           {
	...
	case MT65XX_LED_MODE_GPIO_G: 	
        if( IS_ERR(led1_green_gpio_en0)|| IS_ERR(led1_green_gpio_en1))
        {
	...
	case MT65XX_LED_MODE_GPIO_B: 	
        if( IS_ERR(led2_blue_gpio_en0)|| IS_ERR(led2_blue_gpio_en1))
        {
             pr_err("err: led2_blue_gpio_en0 or led2_blue_gpio_en1 is error!!!");
             return ret;
        }   
        else
         break;
	...
	case MT65XX_LED_MODE_GPIO_KPD: 	
        if( IS_ERR(leds_kpd_gpio_en0)|| IS_ERR(leds_kpd_gpio_en1))
        {
             pr_err("err: leds_kpd_gpio_en0 or leds_kpd_gpio_en1 is error!!!");
             return ret;
        }   
        else
         break;        
	...
	switch(pin)
	{
	case MT65XX_LED_MODE_GPIO_R: 	pinctrl_select_state(pinctrl1, level ? led0_red_gpio_en1 : led0_red_gpio_en0); break; 
	case MT65XX_LED_MODE_GPIO_G: 	pinctrl_select_state(pinctrl1, level ? led1_green_gpio_en1 : led1_green_gpio_en0); break; 
	case MT65XX_LED_MODE_GPIO_B: 	pinctrl_select_state(pinctrl1, level ? led2_blue_gpio_en1 : led2_blue_gpio_en0); break; 
	case MT65XX_LED_MODE_GPIO_KPD: 	pinctrl_select_state(pinctrl1, level ? leds_kpd_gpio_en1 : leds_kpd_gpio_en0); break; 
...
struct of_device_id mtk_leds_of_match[] = {
...
static struct platform_driver mt65xx_leds_driver = {
	.driver = {
		   ...
		   .owner = THIS_MODULE,	   
#ifdef CONFIG_OF
                         .of_match_table = mtk_leds_of_match,
#endif
		   },
