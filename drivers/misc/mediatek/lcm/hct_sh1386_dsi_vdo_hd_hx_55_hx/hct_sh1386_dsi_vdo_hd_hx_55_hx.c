...
static void lcm_power_ic_en(unsigned char enabled)
{
	if (enabled)
	{
		#ifdef GPIO_LCD_ENN_PIN
		    lcm_util.set_gpio_mode(GPIO_LCD_ENN_PIN, GPIO_MODE_00);
		    lcm_util.set_gpio_dir(GPIO_LCD_ENN_PIN, GPIO_DIR_OUT);
		    lcm_util.set_gpio_out(GPIO_LCD_ENN_PIN, GPIO_OUT_ONE);
		#else
	...
	else
	{
		    ...
		    lcm_util.set_gpio_mode(GPIO_LCD_ENN_PIN, GPIO_MODE_00);
		    lcm_util.set_gpio_dir(GPIO_LCD_ENN_PIN, GPIO_DIR_OUT);
		    lcm_util.set_gpio_out(GPIO_LCD_ENN_PIN, GPIO_OUT_ZERO);
		#else
			mt_dsi_pinctrl_set(LCM_POWER_ENN, 0);
		#endif
		//mt_dsi_pinctrl_set(LCM_POWER_ENP, 0); // 为了兼容 A1和A2 的板子
	}
}
...
//		Video mode setting		
  params->dsi.intermediat_buffer_num = 0;	//because DSI/DPI HW design change, this parameters should be 0 when video mode in MT658X; or memory leakage
		...
		params->dsi.horizontal_backporch				= 30;//270; // from Q driver
  		params->dsi.horizontal_frontporch				= 20;// 30// 0x18 ;
		params->dsi.horizontal_active_pixel				= FRAME_WIDTH;

		params->dsi.PLL_CLOCK = 190;//240; //this value must be in MTK suggested table
