...
extern void sub_FL_get_dts_info(const struct of_device_id *maches );
...
struct pinctrl_state *flashlight_sub_mode_l = NULL;
struct pinctrl_state *flashlight_sub_mode_h = NULL;

struct pinctrl_state *flashlight_pins_sub_pwm_l = NULL;
struct pinctrl_state *flashlight_pins_sub_pwm_h = NULL;
struct pinctrl_state *flashlight_pins_sub_as_pwm = NULL;

#if defined(CONFIG_NCP1854_DUAL_SUPPORT)
struct pinctrl_state *dual_switch_sub_en_low = NULL;
struct pinctrl_state *dual_switch_sub_en_high = NULL;

void Dual_Switch_sub_enable(int level)
{
	if(IS_ERR(dual_switch_sub_en_low)||IS_ERR(dual_switch_sub_en_high))
	{
		pr_err( "err: dual_switch_sub_en_low or dual_switch_sub_en_high is error!!!");
		return;
	}   
	if(level)
		pinctrl_select_state(flashlightpinsctrl, dual_switch_sub_en_high);
	else
		pinctrl_select_state(flashlightpinsctrl, dual_switch_sub_en_low);
}
...
void flashlight_gpio_as_pwm(int pin, int config)
{
	...
	if(FL_SUB_PMW_PIN == pin)
      {   
            if(IS_ERR(flashlight_pins_sub_as_pwm))
                return;
            else
		pinctrl_select_state(flashlightpinsctrl, flashlight_pins_sub_as_pwm);
       }
}
...
case FL_SUB_MODE_PIN:	  
	if(IS_ERR(flashlight_sub_mode_l)||IS_ERR(flashlight_sub_mode_h))
	{
...
#endif   
#endif
#ifdef __HCT_SUB_FLASHLIGHT_PWM_SUPPORT__
case FL_SUB_PMW_PIN:	
	if(IS_ERR(flashlight_pins_sub_pwm_h)||IS_ERR(flashlight_pins_sub_pwm_l))
	{
		 pr_err("err: flashlight_pins_sub_pwm_h or flashlight_pins_sub_pwm_l is error!!!");
		 return;
	}	
	else
	 break;
	...
	case FL_SUB_MODE_PIN: 	pinctrl_select_state(flashlightpinsctrl, level ? flashlight_sub_mode_h : flashlight_sub_mode_l); break; 
	case FL_SUB_PMW_PIN:    pinctrl_select_state(flashlightpinsctrl, level ? flashlight_pins_sub_pwm_h : flashlight_pins_sub_pwm_l); break; 
	default: ;
	}
}

int flashlight_dts_init(struct platform_device *pdev)
{
	int ret = 0;
	...
	flashlight_sub_mode_l = pinctrl_lookup_state(flashlightpinsctrl, "flashlight_sub_mode0");
	if (IS_ERR(flashlight_sub_mode_l)) {
		ret = PTR_ERR(flashlight_sub_mode_l);
		pr_debug("%s : pinctrl err, flashlight_sub_mode_l\n", __func__);
	}

	/*flashlight sub en Pin initialization */
	flashlight_sub_mode_h = pinctrl_lookup_state(flashlightpinsctrl, "flashlight_sub_mode1");
	if (IS_ERR(flashlight_sub_mode_h)) {
		ret = PTR_ERR(flashlight_sub_mode_h);
		pr_debug("%s : pinctrl err, flashlight_sub_mode_h\n", __func__);
	}
	flashlight_pins_sub_pwm_l= pinctrl_lookup_state(flashlightpinsctrl, "flashlight_pins_sub_pwm0");
	if (IS_ERR(flashlight_pins_sub_pwm_l)) {
		ret = PTR_ERR(flashlight_pins_sub_pwm_l);
		pr_debug("%s : pinctrl err, flashlight_pins_sub_pwm0\n", __func__);
	}
	flashlight_pins_sub_pwm_h = pinctrl_lookup_state(flashlightpinsctrl, "flashlight_pins_sub_pwm1");
	if (IS_ERR(flashlight_pins_sub_pwm_h)) {
		ret = PTR_ERR(flashlight_pins_sub_pwm_h);
		pr_debug("%s : pinctrl err, flashlight_pins_sub_pwm1\n", __func__);
	}
	flashlight_pins_sub_as_pwm= pinctrl_lookup_state(flashlightpinsctrl, "flashlight_pins_sub_as_pwm");
	if (IS_ERR(flashlight_pins_sub_as_pwm)) {
		ret = PTR_ERR(flashlight_pins_sub_as_pwm);
		pr_debug("%s : pinctrl err, flashlight_pins_sub_as_pwm\n", __func__);
	}

	
#if defined(CONFIG_NCP1854_DUAL_SUPPORT)

	dual_switch_sub_en_low = pinctrl_lookup_state(flashlightpinsctrl, "dual_switch_sub_en0");
	if (IS_ERR(dual_switch_sub_en_low)) {
		ret = PTR_ERR(dual_switch_sub_en_low);
		pr_debug("%s : pinctrl err, dual_switch_sub_en_low\n", __func__);
	}

	/*flashlight sub en Pin initialization */
	dual_switch_sub_en_high = pinctrl_lookup_state(flashlightpinsctrl, "dual_switch_sub_en1");
	if (IS_ERR(dual_switch_sub_en_high)) {
		ret = PTR_ERR(dual_switch_sub_en_high);
		pr_debug("%s : pinctrl err, dual_switch_sub_en_high\n", __func__);
	}
      ...
      sgm3785_get_dts_info(flashlight_of_match);
		...
		sub_FL_get_dts_info(flashlight_of_match);
#endif
