...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = HM5040MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
#define HM5040MIPI_MaxGainIndex (64)
kal_uint32 HM5040MIPI_sensorGainMapping[HM5040MIPI_MaxGainIndex][2] ={
    {64 ,0x00},
    {68 ,0x01},
    {72 ,0x02},
    {76 ,0x03},
...
{
	kal_uint8 iI;
    LOG_INF("[hm5040MIPI]enter hm5040MIPIGain2Reg function\n");
    for (iI = 0; iI < (HM5040MIPI_MaxGainIndex-1); iI++)
...
********************************************************************************/
{
        ...
        if(gain <HM5040MIPI_sensorGainMapping[iI][0])
		{
            break;
       // LOG_INF("[brad_gain] iGain=%d gain_tmp0=%d  analoggain=%d  Reg_Cgain = 0x%x    \n",iGain,gain_tmp0,HM5040_gain_table[i].analoggain,Reg_Cgain);
		...
		if(gain < HM5040MIPI_sensorGainMapping[iI][0])
    {
			return HM5040MIPI_sensorGainMapping[iI][1];
...
//    HM5040MIPI_write_cmos_sensor(0x0104, 0x00); 
    ...
    {
         LOG_INF("Gain mapping don't correctly:%d %d \n", gain, HM5040MIPI_sensorGainMapping[iI][0]);
    }
	LOG_INF("exit hm5040MIPIGain2Reg function\n");
    return HM5040MIPI_sensorGainMapping[iI-1][1];
