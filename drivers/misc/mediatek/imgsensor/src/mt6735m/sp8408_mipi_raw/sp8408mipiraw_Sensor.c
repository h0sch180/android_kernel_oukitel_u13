...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		.pclk = 146670000,				//record different mode's pclk
		.linelength  = 3792,				//record different mode's linelength
...
write_cmos_sensor(0x0821,0x0028);//,MSB_LBRATE[23:16];
write_cmos_sensor(0x0822,0x0000);//,MSB_LBRATE[15:8];
write_cmos_sensor(0x0823,0x0000);//,MSB_LBRATE[7:0];
write_cmos_sensor(0x0301,0x0007);//,-/-/-/VT_PIX_CLK_DIV[4:0];
...
write_cmos_sensor(0x0307,0x00b8);//,PLL_MULTIPLIER[7:0];
write_cmos_sensor(0x030B,0x0002);//,-/-/OP_SYS_CLK_DIV[5:0];
...
write_cmos_sensor(0x0341,0x00fe);//,FR_LENGTH_LINES[7:0];
write_cmos_sensor(0x0342,0x000e);//,LINE_LENGTH_PCK[15:8];
write_cmos_sensor(0x0343,0x00d0);//,LINE_LENGTH_PCK[7:0];
...
write_cmos_sensor(0x0341,0x00fc);//,FR_LENGTH_LINES[7:0];
write_cmos_sensor(0x0342,0x000e);//,LINE_LENGTH_PCK[15:8];
write_cmos_sensor(0x0343,0x00d0);//,LINE_LENGTH_PCK[7:0];
write_cmos_sensor(0x0344,0x0000);//,X_ADDR_START[15:8];
write_cmos_sensor(0x0345,0x0000);//,X_ADDR_START[7:0];
