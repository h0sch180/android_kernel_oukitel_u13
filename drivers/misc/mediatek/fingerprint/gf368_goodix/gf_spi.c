...
FUNC_ENTRY();
	...
	//pr_info("begin key = %d.\n",gf_key.key);
	//pr_info("begin key_back = %d.\n",KEY_BACK);
	//pr_info("begin key.val = %d.\n",gf_key.value);	
	if (gf_key.key == KEY_BACK || (gf_key.key == KEY_HOME))
	{
...
static void gf_reg_key_kernel(struct gf_dev *gf_dev)
{
    ...
    __set_bit(GF_KEY_F11, gf_dev->input->keybit);
