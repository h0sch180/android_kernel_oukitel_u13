...
struct pinctrl *hct_irtx_pinctrl;
struct pinctrl_state *hct_irtx_power_on, *hct_irtx_power_off;

int hct_irtx_get_gpio_info(struct platform_device *pdev)
{
	struct device_node *node;
	int ret;
	node = of_find_compatible_node(NULL, NULL, "mediatek,irtx");
	printk("node.name %s full name %s",node->name,node->full_name);


	hct_irtx_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(hct_irtx_pinctrl)) {
		ret = PTR_ERR(hct_irtx_pinctrl);
		dev_err(&pdev->dev, "hct_irtx cannot find pinctrl\n");
		return ret;
	}

	printk("[%s] hct_irtx_pinctrl+++++++++++++++++\n",pdev->name);

	hct_irtx_power_on = pinctrl_lookup_state(hct_irtx_pinctrl, "irtx_pwr_en1");
	if (IS_ERR(hct_irtx_power_on)) {
		ret = PTR_ERR(hct_irtx_power_on);
		dev_err(&pdev->dev, " Cannot find hct_irtx pinctrl hct_irtx_power_on!\n");
		return ret;
	}
	hct_irtx_power_off = pinctrl_lookup_state(hct_irtx_pinctrl, "irtx_pwr_en0");
	if (IS_ERR(hct_irtx_power_off)) {
		ret = PTR_ERR(hct_irtx_power_off);
		dev_err(&pdev->dev, " Cannot find hct_irtx pinctrl hct_irtx_power_off!\n");
		return ret;
	}
	printk("hct_irtx get gpio info ok--------");
	return 0;
}

int hct_irtx_set_power(int cmd)
{

	if(IS_ERR(hct_irtx_power_off)||IS_ERR(hct_irtx_power_on))
	{
		 pr_err( "err: hct_irtx_power_on or hct_irtx_power_off is error!!!");
		 return -1;
	}	
	...
	{
	case 0 : 		
		pinctrl_select_state(hct_irtx_pinctrl, hct_irtx_power_off);
	break;
	case 1 : 		
		pinctrl_select_state(hct_irtx_pinctrl, hct_irtx_power_on);
	break;
	}
	return 0;
}
...
static int irtx_probe(struct platform_device *plat_dev)
{
	...
	u32 major = 0, minor = 0;
	int ret = 0;
	hct_irtx_get_gpio_info(plat_dev);
#ifdef CONFIG_OF
