...
static u8 *_gFwVersion_test = NULL; 
...
//    const char *vcc_i2c_name = "vcc_i2c";
    ...
    if (ret == 0) // If probe is success, then enable the below flag.
    {
      ...
      DrvIcFwLyrGetCustomerFirmwareVersion(&nMajor, &nMinor, &_gFwVersion_test);
	  sprintf(tpd_device_driver.descript,"Vno: M%dS%d", nMajor,nMinor);
