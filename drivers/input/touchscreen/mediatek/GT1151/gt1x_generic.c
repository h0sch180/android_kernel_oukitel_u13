...
s32 gt1x_read_version(struct gt1x_version_info *ver_info)
{
	...
	while (retry--) {
		...
		IS_NUM_OR_CHAR(buf[0]) && IS_NUM_OR_CHAR(buf[1]) && IS_NUM_OR_CHAR(buf[2]) && buf[10] != 0xFF) {
			...
			GTP_ERROR("GTP read version failed!(checksum error-%d,%d,%d,%d,%d)",checksum,buf[0],buf[1],buf[2],buf[10] );
		} else {
			GTP_ERROR("GTP read version failed!~~~i2c error");
		}
...
error_exit:
      ...
      GTP_ERROR("GTP gt1x_init failed.");
	return ret;
}
