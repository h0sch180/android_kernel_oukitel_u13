...
#endif
{
	...
	msleep(DELAY_S7300_RESET_READY);
#endif
	TPD_DEBUG("synaptics_rmi4_probe() enter!\n");
...
#if 1  // hct_add power control by lcm
void tpd_vgp_reg_regulator_disable(void)
{
	int ret;
	...
	ret = regulator_disable(tpd->reg);	/* disable regulator */
	if (ret)
		TPD_DMESG("tpd regulator_disable() failed!\n");
}
void tpd_vgp_reg_regulator_enable(void)
{
	int ret;
	if(exp_data.rmi4_data==NULL)
		return ;

	ret = regulator_enable(tpd->reg);	/* disable regulator */
	if (ret)
		TPD_DMESG("tpd regulator_enable() failed!\n");
}
#endif
...
{
	int retval;
	TPD_DEBUG("synaptics I2C Touchscreen Driver (Built )\n");
