...
int aw9136_sensor_get_dts_info(struct i2c_client *client)
{
    ...
    /* gpio setting */
        mtk_tpd_pinctrl = devm_pinctrl_get(&mtk_tp_pdev->dev);
...
static int AW9136_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  /*F3 used as power key, others as sliding focus in camera*/
	...
	AW9136_ts->irq_node = of_find_compatible_node(NULL, NULL, "mediatek,aw9136_ts_int");
